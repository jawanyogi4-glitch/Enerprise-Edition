...
//     };
//   } catch (error) {
//     console.warn("Mocking auth metadata due to backend not available");
...
// export const getAuthTypeMetadataSS = async (): Promise<AuthTypeMetadata> => {
//   const res = await fetch(buildUrl("http://localhost:8000/auth/type")); ///auth/type
//   if (!res.ok) {
//     throw new Error("Failed to fetch data");
//   }
...
//   // qilegal in an un-authenticated state
...
export const getCurrentUserSS = async (): Promise<User | null> => {
  ...
  return {
    id: "1",
    email: "yogesh.chourasiya@qilegal.com",
    is_active: true,
