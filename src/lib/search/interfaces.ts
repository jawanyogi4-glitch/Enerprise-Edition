...
export interface MinimalqilegalDocument {
  document_id: string;
  semantic_identifier: string | null;
}

export interface qilegalDocument extends MinimalqilegalDocument {
  link: string;
  source_type: ValidSources;
  blurb: string;
  boost: number;
...
export interface LoadedqilegalDocument extends qilegalDocument {
  icon: React.FC<{ size?: number; className?: string }>;
}

export interface SearchqilegalDocument extends qilegalDocument {
  is_relevant: boolean;
  relevance_explanation: string;
}

export interface FilteredqilegalDocument extends qilegalDocument {
  included: boolean;
}
export interface DocumentInfoPacket {
  top_documents: qilegalDocument[];
...
export interface SearchResponse {
  ...
  answer: string | null;
  quotes: Quote[] | null;
  documents: SearchqilegalDocument[] | null;
...
export interface SearchRequestArgs {
  ...
  updateDocs: (documents: qilegalDocument[]) => void;
