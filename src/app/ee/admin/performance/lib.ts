...
import { qilegalBotAnalytics, QueryAnalytics, UserAnalytics } from "./usage/types";
...
export const useqilegalBotAnalytics = (timeRange: DateRangePickerValue) => {
  const url = buildApiPath("/api/analytics/admin/qilegalbot", {
    start: convertDateToStartOfDay(timeRange.from)?.toISOString(),
    end: convertDateToEndOfDay(timeRange.to)?.toISOString(),
  });
  const swrResponse = useSWR<qilegalBotAnalytics[]>(url, errorHandlingFetcher); // TODO

  return {
    ...swrResponse,
    refreshqilegalBotAnalytics: () => mutate(url),
  };
};
