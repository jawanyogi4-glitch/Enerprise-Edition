...
export default function AIMessage({
  ...
  const documentMapRef = useRef<Map<string, qilegalDocument>>(new Map());
  ...
  // Rotating thinking messages for Case Analysis
  const thinkingSteps = [
    "Thinking…",
    "Analyzing case details…",
    "Reviewing legal precedents…",
    "Building reasoning…",
    "Examining factual patterns…",
    "Assessing legal arguments…",
    "Cross-checking case law…",
    "Identifying applicable provisions…",
    "Mapping legal issues…",
    "Weighing strengths and weaknesses…",
    "Reviewing judicial interpretations…",
    "Considering counterarguments…",
    "Checking fact-to-law alignment…",
    "Determining legal viability…",
    "Interpreting statutory intent…",
    "Reconciling conflicting precedents…",
    "Assessing burden of proof…",
    "Formulating legal conclusion…",
    "Preparing structured analysis…",
  ];

  // Rotating loading messages for Legacy Search (assistant id = 2)
  const legacySearchThinkingSteps = [
    "Retrieving relevant documents…",
    "Scanning indexed sources…",
    "Extracting high-relevance entries…",
    "Cross-matching search parameters…",
    "Collecting authoritative references…",
    "Preparing structured search summary…",
  ];

  const [legacyThinkingIndex, setLegacyThinkingIndex] = useState(0);
  const [thinkingIndex, setThinkingIndex] = useState(0);

  useEffect(() => {
    if (chatState.assistant?.id !== 1) return;
    if (groupedPacketsRef.current.length !== 0) return;
  ...
  }, [chatState.assistant?.id, groupedPacketsRef.current.length]);

  // Rotating messages for Legacy Search (assistant id = 2)
  useEffect(() => {
    if (chatState.assistant?.id !== 2) return;
    if (groupedPacketsRef.current.length !== 0) return;

    const interval = setInterval(() => {
      setLegacyThinkingIndex(
        (prev) => (prev + 1) % legacySearchThinkingSteps.length
      );
    }, 6000);
  ...
  return (
    <>
      ...
      <div
        // for e2e tests
        data-testid={displayComplete ? "qilegal-ai-message" : undefined}
                    ...
                    <div className="max-w-message-max break-words">
                      ...
                      >
                        {groupedPackets.length === 0 ? (
                          chatState.assistant?.id === 1 ? (
                            // CASE ANALYSIS THINKING
                            <span className="ml-3 inline-flex items-center text-base font-semibold text-text-500 animate-pulse">
                              {thinkingSteps[thinkingIndex]}
                            </span>
                          ) : chatState.assistant?.id === 2 ? (
                            // LEGACY SEARCH THINKING
                            <span className="ml-3 inline-flex items-center text-base font-semibold text-text-500 animate-pulse">
                              {legacySearchThinkingSteps[legacyThinkingIndex]}
                            </span>
                          ) : (
                            // DEFAULT THINKING FOR OTHER AGENTS
