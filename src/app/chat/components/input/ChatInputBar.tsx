...
export interface ChatInputBarProps {
  ...
  setPresentingDocument?: (document: MinimalqilegalDocument) => void;
  toggleDeepResearch: () => void;
  disabled: boolean;
  onOpenLegacySearchConfig?: () => void;
}
...
function ChatInputBarInner({
  ...
  // Convert ProjectFile to MinimalqilegalDocument format for viewing
  const handleFileClick = useCallback(
    (file: ProjectFile) => {
      if (!setPresentingDocument) return;

      const documentForViewer: MinimalqilegalDocument = {
    ...
    >
      ...
      <div className="w-full h-full flex flex-col shadow-01 bg-background-neutral-00 rounded-16">
        ...
        <textarea
          ...
          ref={textAreaRef}
          id="qilegal-chat-input-textarea"
          className={cn(
            "w-full",
            "outline-none",
            "bg-transparent",
        ...
        <div className="flex justify-between items-center w-full p-1">
          <div className="flex flex-row items-center">
            {/* File Upload / Legacy Search Config / Case Analysis Disabled */}
            {(() => {
              const isCaseAnalysis =
                selectedAssistant?.id === ASSISTANT_ID_CASE_ANALYSIS ||
                selectedAssistant?.name === "Case Analysis";

              const isLegacySearch =
                selectedAssistant?.id === ASSISTANT_ID_LEGACY_SEARCH ||
                selectedAssistant?.name === "Legacy Search";

              // 1. CASE ANALYSIS → hide file upload completely
              if (isCaseAnalysis) {
                return null; // This will hide the button entirely
              }

              // 2. LEGACY SEARCH → open Search Configuration popup
              if (isLegacySearch) {
                return (
                  <IconButton
                    icon={SvgPlusCircle}
                    tooltip="Search Configuration"
                    onClick={onOpenLegacySearchConfig}
                    disabled={disabled}
                  />
                );
              }

              // 3. ALL OTHER ASSISTANTS → normal file upload popover
              return (
                <FilePickerPopover
                  onFileClick={handleFileClick}
          ...
          <div className="flex flex-row items-center gap-1">
            {/* Hide model selector for Case Analysis + Legacy Search */}
            {!(
              selectedAssistant?.id === 1 ||
              selectedAssistant?.name === "Case Analysis" ||
              selectedAssistant?.id === 2 ||
              selectedAssistant?.name === "Legacy Search"
            ) && (
            ...
            <IconButton
              id="qilegal-chat-input-send-button"
