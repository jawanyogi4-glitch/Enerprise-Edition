...
const Toast = ({
  ...
  return (
    <div
      className={`fixed top-4 right-4 bg-green-500 text-white shadow-lg rounded-lg px-6 py-4 flex items-center space-x-4 ${
    ...
    <>
      <div className="flex items-center justify-center min-h-screen bg-[#fafafa] dark:bg-gray-900">
        ...
        <div className="w-full max-w-7xl p-16 bg-white dark:bg-gray-800 text-black dark:text-white rounded-lg shadow-lg">
          ...
          {mergeNotification && (
            <div
              className="fixed top-16 left-1/2 transform -translate-x-1/2 z-50 bg-green-600 text-white text-lg font-medium px-8 py-4 rounded-md shadow-xl transition-transform transition-opacity duration-500 ease-out"
              style={{
                animation: "fadeSlideInOut 3s forwards",
              }}
            >
          ...
          <div className="mb-8">
            ...
            <div className="mt-6 flex justify-between">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg shadow-md transition-all duration-300 ease-in-out transform hover:scale-105 ${
                  isCancelDisabled
                    ? "bg-gray-300 text-gray-500 cursor-not-allowed"
            ...
            <div ref={documentSectionsRef} className="mb-8">
              ...
              <div className="flex justify-between mt-4">
                <button
                  onClick={async () => {
                    try {
                      ...
                      setToastMessage(
                        response.data.message || "Titles saved successfully!"
                      );
                    } catch {
                      setToastMessage(
                        "Failed to save titles. Please try again."
                      );
                    } finally {
                ...
                <button
                  ...
                  className={`px-4 py-2 text-white rounded-lg shadow-md focus:outline-none ${
                    isFetchingGeneratedDocument ? "bg-red-500" : "bg-green-500"
                  } hover:bg-green-600`}
                >
              ...
              {generatedDocument.map((section, index) => (
                <div key={index} className="mb-6">
                  ...
                  <div
                    aria-live="polite"
                    aria-atomic="true"
                    className={`fixed bottom-4 right-4 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg transition-all duration-300 ease-in-out ${
                      copiedStatus[index]?.content
                  ...
                  >
                    {copiedStatus[index]?.content && (
                      <span>Content copied successfully!</span>
                    )}
                  </div>
                </div>
              ))}
