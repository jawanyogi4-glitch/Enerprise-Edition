...
import { qilegalApiClient } from "../utils/qilegalApiClient";
  ...
  test.describe("With Knowledge", () => {
    ...
    test.afterAll(async ({ browser }: { browser: Browser }) => {
      ...
      if (ccPairId && documentSetId) {
        ...
        const cleanupClient = new qilegalApiClient(page);
    ...
    test("should create and edit assistant with Knowledge enabled", async ({
      ...
      const qilegalApiClient = new qilegalApiClient(page);
      ccPairId = await qilegalApiClient.createFileConnector("Test Connector");
      documentSetId = await qilegalApiClient.createDocumentSet(
        "Test Document Set",
        [ccPairId]
      );
