...
/**
 * API Client for qilegal backend operations in E2E tests.
 *
 * Provides a type-safe, abstracted interface for interacting with the qilegal backend API.
 ...
 * **Usage Example:**
 * ```typescript
 * const client = new qilegalApiClient(page);
...
export class qilegalApiClient {
  ...
  private async handleResponseSoft(
    ...
    if (!response.ok()) {
      const errorText = await response.text();
      console.error(
        `[qilegalApiClient] ${errorMessage}: ${response.status()} - ${errorText}`
      );
      return false;
    }
    return true;
   ...
   *
   * @param message - The message to log (will be prefixed with "[qilegalApiClient]")
   */
  private log(message: string): void {
    console.log(`[qilegalApiClient] ${message}`);
  }
